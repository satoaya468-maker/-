...
/* Ссылка на группу или чат ВКонтакте, например https://vk.com/fistashka_mgn
   или https://vk.me/fistashka_mgn для сообщений сообщества.
   Пока строка пуста, кнопки «Написать в ВК» и слайд про группу с
   страницы убираются: мёртвая ссылка хуже, чем её отсутствие. */
const VK_URL = 'https://vk.ru/FistashkaProduct';
...
(function () {
  ...
  /* ---------------------------------------------------------
     3а. Ссылки на ВКонтакте.
         Выполняется до карусели: если ссылки нет, слайд про группу
         удаляется, и карусель уже не строит для него точку.
     --------------------------------------------------------- */
  document.querySelectorAll('[data-vk]').forEach(function (el) {
    if (VK_URL) {
      el.setAttribute('href', VK_URL);
      el.setAttribute('target', '_blank');
      el.setAttribute('rel', 'noopener');
      return;
    }
    // слайд целиком про ВК: без ссылки он бессмысленен
    const slide = el.closest('.promo__slide');
    (slide || el).remove();
  });
  ...
  (function initPromo() {
    ...
    // подпись «N из M» проставляем здесь: слайд про ВК мог быть удалён,
    // и захардкоженные в разметке номера разъехались бы
